...
export class TableStatements {
    ...
    public getInsertOrReplaceStatement(): DatabaseStatement {
        if (!this.insertOrReplaceStatement) {
            this.rdb = new dataRdb.RdbPredicates(this.tablename);
    ...
    public getDeleteStatement(): DatabaseStatement {
        if (!this.deleteStatement) {
            this.rdb = new dataRdb.RdbPredicates(this.tablename);
            let newDeleteStatement: DatabaseStatement = this.db.compileStatement(this.rdb,this.tablename);
    ...
    public getUpdateStatement(): DatabaseStatement{
        if (!this.updateStatement) {
            this.rdb = new dataRdb.RdbPredicates(this.tablename)
            let newUpdateStatement: DatabaseStatement = this.db.compileStatement(this.rdb, this.tablename);
