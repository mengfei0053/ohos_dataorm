...
export class Property {
  ...
  private checkArrayNull(...arr: any[]): boolean {
    arr = arr.flat(Infinity);
    if (arr.length !== 0 && Array.from(arr ?? []).length !== 0) {
      return true;
    } else {
      DaoLog.e("Array is null, please check")
      return false;
    }
  }
  ...
  public inData(...inValues: any[]): WhereCondition {
    if (this.checkArrayNull(inValues)) {
  ...
  public inDataSql(...inValues: any[]): WhereCondition {
    if (this.checkArrayNull(inValues)) {
      ...
      SqlUtils.appendPlaceholders(condition.toString(), inValues.length);
  ...
  public notIn(...notInValues: any[]): WhereCondition {
    if (this.checkArrayNull(notInValues)) {
  ...
  public notInSql(...notInValues: any[]): WhereCondition {
    if (this.checkArrayNull(notInValues)) {
      ...
      SqlUtils.appendPlaceholders(condition.toString(), notInValues.length);
