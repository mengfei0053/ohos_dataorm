...
import { getClsTableInfo } from './annotation/Util';
...
export class DbUtils {
  ...
  public static readRawFileSync(resourceMgr: any, filename: string): Uint8Array {
    let per: Uint8Array = resourceMgr.getRawFileContentSync(filename);
    return per;
  }
  ...
  public static rawFile2StrSync(resourceMgr: any, filename: string): string {
    let result: string = '';
    let arr = this.readRawFileSync(resourceMgr, filename);
    result = Unit8ArrayUtils.Uint8ArrayToString(arr);
    return result;
  }
  ...
  public static async executeSqlStatementsInTx(db: Database, statements: string[]): Promise<number> {
    let e;
    let transaction: Transaction|null|undefined = null;
    try {
      transaction = await db.beginTransactionAsync();
      let count: number = await this.executeSqlStatements(db, statements);
      await db.endTransactionAsync(transaction);
      return count;
    } catch (err) {
      DaoTraceUtil.startError("DbUtils.executeSqlStatementsInTx.error", DaoTraceSpace.TraceType.CRUD)
      DaoLog.e(err.message, err.stack)
      await db.rollBackAsync(transaction);
      e = err;
      DaoTraceUtil.finish("DbUtils.executeSqlStatementsInTx.error")
    }
    if (e) {
      throw e;
    }
  ...
  public static getEntityClassName(entityCls: any): string {
    return getClsTableInfo(entityCls).entityClass;
  }
}
