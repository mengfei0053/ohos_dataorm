...
export class QueryBuilder<T> {
  ...
  protected constructor(dao: AbstractDao<T, any>, tablePrefix?: string) {
    ...
    this.tablePrefix = tablePrefix ?? "T";
  ...
  public stringOrderCollation(stringOrderCollation: string): QueryBuilder<T> {
    this.stringOrderCollations = !stringOrderCollation || stringOrderCollation.startsWith(" ") ?
  ...
  protected append(builder: StringBuilder, property: Property): StringBuilder {
    ...
    builder.append(`${this.tablePrefix}.'${property.columnName}'`);
    return builder;
  }
  ...
  private createSelectSql(): string {
    let select =
      SqlUtils.createSqlSelect(this.dao.getTableName(), this.tablePrefix, this.dao.getAllColumns(), this.isDistinct);
    let builder = new StringBuilder(select);
    this.appendJoinAndWheres(builder, this.tablePrefix);
    if (this.orderBuilder && this.orderBuilder.toString().length > 0) {
      builder.append(" ORDER BY ", this.orderBuilder.toString());
    }
  ...
  private checkAddOffset(): number {
    ...
    if (this.offsets!==null && this.offsets !== undefined) {
      if (this.limits === null || this.limits === undefined) {
  ...
  private appendJoinAndWheres(builder: StringBuilder, tablePrefixOrNull: string) {
    ...
    for (let i = 0; i < this.joins.length; i++) {
      ...
      builder.append(` JOIN "${join.daoDestination.getTableName()}" ${join.tablePrefix} ON `);
      let t1 = SqlUtils.appendProperty("", join.sourceTablePrefix, join.joinPropertySource);
  ...
  private appendJoinsAndWheres() {
    ...
    let whereAppended: boolean = this.whereCollector ? true : false;
    if (whereAppended) {
      this.whereCollector.appendWhereClause();
    }
    for (let i = 0; i < this.joins.length; i++) {
      if (this.joins[i].whereCollector) {
  ...
  public listSync(): Array<T> {
    return this.build().listSync();
  }
  ...
  public listSqlSync(): Array<T> {
    return this.build().listSqlSync();
  }
   ...
   * @useinstead QueryBuilder#uniqueAsync
   */
  public unique(): T {
    return this.build().unique();
  }

  public uniqueAsync(): Promise<T> {
    return this.build().uniqueAsync();
  }
  public uniqueSync(): T {
    return this.build().uniqueSync();
  }
  ...
  public uniqueOrThrowAsync(): Promise<T> {
    return this.build().uniqueOrThrowAsync();
  }

  public uniqueOrThrowSync(): T {
    return this.build().uniqueOrThrowSync();
  }
  ...
  public countSync(): number {
    return this.buildCount().countSync();
  }
}