...
export class Query<T> extends AbstractQueryWithLimit<T> {
  ...
  public listSync(): Array<T> {
    let cursor: any
    if (typeof this.predicates === "string") {
      cursor = this.dao.getDatabase().rawQueriesSync(<string> this.predicates, this.dao.getAllColumns())
    } else {
      cursor = this.dao.getDatabase().rawQuerySync(<dataRdb.RdbPredicates> this.predicates, this.dao.getAllColumns());
    }
    return this.daoAccess.loadAllAndCloseCursor(cursor);
  }
  ...
  public listSqlSync(): Array<T> {
    let cursor: any = this.dao.getDatabase().rawQueriesSync(<string> this.predicates, this.parameters);
    return this.daoAccess.loadAllAndCloseCursor(cursor);
  }
  ...
  public uniqueSync():T{
    let cursor: any
    if (typeof this.predicates === "string") {
      cursor = this.dao.getDatabase().rawQueriesSync(<string> this.predicates, this.parameters)
    } else {
      cursor = this.dao.getDatabase().rawQuerySync(<dataRdb.RdbPredicates> this.predicates, this.parameters);
    }
    return this.daoAccess.loadUniqueAndCloseCursor(cursor);
  }
  ...
  public async uniqueOrThrowAsync(): Promise<T> {
    let entity: T = await this.uniqueAsync();
    if (!entity) {
      throw new Error("No entity found for query");
    }
    return entity;
