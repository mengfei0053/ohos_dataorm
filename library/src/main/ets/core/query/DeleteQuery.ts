...
export class DeleteQuery<T> extends AbstractQuery<T> {
  static create<T2>(dao: AbstractDao<T2, any>, predicates: dataRdb.RdbPredicates | string,
    initialValues: Object[]): DeleteQuery<T2> {
  ...
  constructor(queryData: QueryData<T>, dao: AbstractDao<T, any>, predicates: dataRdb.RdbPredicates | string,
  ...
  public async executeDeleteWithoutDetachingEntities() {
    ...
    try {
      let id = await db.Delete(null, <dataRdb.RdbPredicates> this.predicates);
    } catch (e) {
      DaoLog.e(e.message, e.stack)
    }
  }

  public executeDeleteWithoutDetachingEntitiesSync() {
    let db: Database = this.dao.getDatabase();

    // Do TX to acquire a connection before locking this to avoid deadlocks
    // Locking order as described in AbstractDao
    try {
      let id = db.DeleteSync(null, <dataRdb.RdbPredicates> this.predicates);
    } catch (e) {
      DaoLog.e(e.message, e.stack)
    }
  }
