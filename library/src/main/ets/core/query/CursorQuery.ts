...
export class CursorQuery<T> extends AbstractQueryWithLimit<T> {
  ...
  public listSync(): Array<T> {
    let resultSet: dataRdb.ResultSet = this.querySync();
    if (this.daoAccess) {
      let array: Array<T> = this.daoAccess.loadAllAndCloseCursor(resultSet);
      return array
    } else {
      throw new Error("an exception occurred in the listSync method");
    }
  }
  ...
  public querySync(): dataRdb.ResultSet {
    return typeof this.predicates === 'string' ? this.dao.getDatabase().rawQueriesSync(<string> this.predicates, this.parameters) :
       this.dao.getDatabase().rawQuerySync(<dataRdb.RdbPredicates> this.predicates, this.parameters);
  }
