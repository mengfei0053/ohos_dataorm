...
export class CountQuery<T> extends AbstractQuery<T> {
  ...
  public countSync(): number {
    let cursor: any = typeof this.predicates === 'string' ? this.dao.getDatabase().rawQueriesSync(<string> this.predicates) : this.dao.getDatabase().rawQuerySync(<dataRdb.RdbPredicates> this.predicates);
    try {
      return cursor.rowCount;
    } finally {
      cursor.close();
