...
export abstract class DatabaseOpenHelper extends SQLiteOpenHelper {
  ...
  async setAutoMigrate(flag: boolean){
    await StorageUtils.putValue(`${this.name}_autoMigration`, true)
  }

  setAutoMigrateSync(flag: boolean){
    StorageUtils.putValueSync(`${this.name}_autoMigration`, true)
  }

  async getAutoMigrate(){
    return await StorageUtils.getValueByKey(`${this.name}_autoMigration`, false)
  }

  getAutoMigrateSync(){
    return StorageUtils.getValueByKeySync(`${this.name}_autoMigration`, false)
  }
  ...
  async setVersion(version: number): Promise<void> {
    this.versions = await StorageUtils.getValueByKey(this.name + "_dbVersion", 1)
    if (version !== this.versions) {
      if(version>this.versions){
        await StorageUtils.putValue(`${this.name}_upgrade`, true)
      }
      this.versions = version;
      this.mNewVersion = version;
      await StorageUtils.putValue(this.name + "_dbVersion", this.versions);
  ...
  private isCreateDB<T>(t: T): boolean {
    return getClsTableInfo(t).createInDb??false;
  }
