...
export class Student {
  ...
  @Columns({ columnName: 'CID', types: ColumnType.num })
  cId: number

  @ToOne({ joinProperty: 'TID', targetObj: Teacher })
  teacher: Teacher

  @ToOne({ joinProperty: 'CID', targetObj: Classes })
  classes: Classes

  constructor(id?: number, name?: string, tId?: number, cId?: number, teacher: Teacher = new Teacher(), classes: Classes = new Classes()) {
    this.id = id;
    this.name = name;
    this.tId = tId;
    this.cId = cId;
